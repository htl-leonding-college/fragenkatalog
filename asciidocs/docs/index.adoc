...
[%collapsible]
====
Ein Eintrag in `.gitignore` sorgt dafür, dass bestimmte Dateien nicht mit ins Git Repository aufgenommen werden. Das ist wichtig, weil man sonst private Daten (z.B. in `.env` Dateien) oder Dependencies wie `node_modules` hochladen würde, wodurch `git push` und `git pull` langsamer sind.
====
