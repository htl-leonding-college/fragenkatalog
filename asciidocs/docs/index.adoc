...
[%collapsible]
====
- Ein github Template Repo dient als Vorlage,
um daraus neue Repositories mit derselben Struktur und Startdateien zu erstellen.
====
...
=== git fork [.badge.foundation]#Foundation#
- Was ist ein git fork?

.Lösung
[%collapsible]
====
- Eine Kopie eines bestehenden Git-Repositories, die unter dem eigenen Account liegt.
====

- Welchen Zweck erfüllt ein fork?

.Lösung
[%collapsible]
====
- Man macht einen Fork, damit man an einem fremden Repository arbeiten kann,
ohne das Original zu verändern, z.B um Änderungen auszuprobieren, Fehler zu beheben oder Verbesserungen vorzuschalgen(Pull request).
====

=== Pull request [.badge.foundation]#Foundation#
- Was ist ein Pull Request?

.Lösung
[%collapsible]
====
- Ein Pull Request ist eine Anfrage, mit der man eigene Änderungen in ein anderes Repository übernehmen lassen möchte.
====
...
[%collapsible]
====
. Fork des Original-Repositories erstellen.
. Einen Branch im Fork erstellen und dort Änderungen durchführen.
. Änderungen committen und pushen.
. Pull Request auf GitHub erstellen.

https://youtu.be/8lGpZkjnkt4?si=YiNkn7HuJjY6L-Rz[GitHub Pull Request in 100 Seconds]
====


=== gh-pages [.badge.foundation]#Foundation#

- Wie ist GitHub Pages?

.Lösung
[%collapsible]
====
- GitHub Pages ist ein Dienst von GitHub, mit dem man statische Webseiten direkt aus einem Repository kostenlos veröffentlichen und hosten kann.
====
...
=== Alle Docker-Elemente löschen [.badge.foundation]#Foundation#
...
[source,shell,linenum,highlight=19]
----
#!/bin/bash
...
[source,shell,linenum,highlight=29]
----
#!/bin/bash
...
* Was ist der Unterschied zwischen `docker compose stop` and `docker compose down`?
...
[%collapsible]
====
* Beim *imperativen Management* werden die Kommandos direkt ausgeführt, um den gewünschten Zustand zu erreichen (z.B. `kubectl create deployment ...`).
Vergleichbar mit Programmieren: Befehle werden nacheinander ausgeführt.
_Es wird der Weg zur Erreichung des gewünschten Zustands definiert._
* Beim *deklarativen Management* definiert man den gewünschten Zustand in einer YAML-Datei (Manifest) und wenden diese mit `kubectl apply -f ...` an.
Kubernetes sorgt dann dafür, dass der tatsächliche Zustand dem gewünschten Zustand entspricht.
_Es wird nur der Zielzustand definiert, nicht der Weg dorthin._
====
...
=== Was ist ein Reverse Proxy? [.badge.foundation]#Foundation#
...
https://htl-leonding-college.github.io/quarkus-jar-gh-actions-demo/nginx-reverse-proxy.html[Tutorial: nginx as Reverse Proxy^]


Ein Reverse Proxy ist ein Server, der Anfragen aus dem Internet entgegennimmt und sie an interne Server weiterleitet. Er fungiert als „Schutzschild“ und „Verteiler“ vor deiner eigentlichen Infrastruktur.
Die Kernfunktionen
...
=== Wie kann man sich die derzeitigen Cronjobs anzeigen lassen? [.badge.foundation]#Foundation#
...
=== Wie kann man sich die derzeitigen Cronjobs editieren? [.badge.foundation]#Foundation#
...
xref:agentic-programming.adoc[Zum Kapitel: Agentisches Programmieren]

== GitHub Classroom

=== Wie kann man ein GitHub Classroom Assignment erstellen? [.badge.foundation]#Foundation#

.Lösung
[%collapsible%open]
====
. Erstellen eines Repositories auf GitHub, das als Vorlage für das Assignment dienen soll. In diesem Repo erstellt die Lehrkraft zB die Angabe für eine Leistungsfeststellung. Dabei ist zu beachten:
.. Dieses Repo muss sich in der GitHub-Organisation befinden, in der die Assignments erstellt werden sollen. z.B. Organisation `2526-4ahitm-itp`, Repository-Name: `01-lab-keycloak`
.. Das Repository `01-lab-keycloak` muss ein Template-Repository sein.

. Anschließend wird ein neues Assignment in GitHub Classroom erstellt.
Das Template-Repository `01-lab-keycloak` wird Vorlage ausgewählt.

image::github-classroom-assignment.png[]



====

=== Wie kann man in ein GitHub Classroom Assignment, das bereits von Schülern angenommen wurde, Änderungen einpflegen? [.badge.foundation]#Foundation#

.Lösung
[%collapsible%open]
====
. Änderungen im Template-Repository vornehmen, zB `01-lab-keycloak`
. Anschließend das vom Template-Repository abgeleitete Repository clonen, zB `2526-4ahitm-itp-01-lab-keycloak-01-lab-keycloak`
...
. Nun können die Änderungen aus dem Template-Repository in das Original-Repo zB `2526-4ahitm-itp-01-lab-keycloak-01-lab-keycloak` geholt werden, von dem die Schüler das Assignment angenommen (fork) haben:
+
[source,shell]
----
git fetch template
...
----
+
IMPORTANT: Es muss `--allow-unrelated-histories` angegeben werden, da die Historie des Template-Repositories und des Original-Repositories nicht zusammengeführt werden kann, da sie unterschiedliche Wurzeln haben. Es handelt sich ja um zwei verschiedene Repositories, die nicht voneinander abgeleitet sind, sondern nur den gleichen Code enthalten.
...
+
image::github-classroom-sync-repos.png[]
