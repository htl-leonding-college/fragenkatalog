...
|HEAD^ Der Vorgänger (also ein Commit zurück)
|HEAD~1 ist der eine Commit davor, also der vorletzte.
...
IMPORTANT: _Grundsatz_: Dateien, die generiert werden (zB duch kompilieren, durch npm install), sollten nicht im Repository sein. Dateien, die nicht erstellt werden (zB *Konfigurationsdateien*, *Dokumentation*, *Programmcode*) sind im Repository zu speichern. *Aber keine privaten Daten wie Passwörter, API-Keys, etc.!*
====
